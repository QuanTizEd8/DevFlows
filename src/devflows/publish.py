...
# devflows-scenarios.yaml (~109 KB) runs fine, a 186 KB anaconda-publish.yaml was
# rejected at startup, and a 135 KB split of it still failed -- so the real limit sits
# just above 109 KB and at or below 135 KB. This cap is set below the known-bad 135 KB
# class and above the largest legitimate generated workflow (devflows-scenarios at
# ~109 KB) so that a bloat regression -- most likely a shared script re-inlined into
# every job -- fails `devflows sync` locally instead of silently at startup.
# anaconda-publish itself is held under 100 KB (see tests/test_anaconda_publish.py).
# If a legitimate workflow ever approaches this cap, shrink it (split or trim its
# inlined scripts) rather than raising the cap.

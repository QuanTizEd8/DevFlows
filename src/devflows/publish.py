...
# Hard byte cap on a generated workflow file, applied to BOTH published catalog
# workflows and the generated scenario workflows (see scenarios.py). GitHub rejects
# an oversized workflow at startup with an opaque "workflow file issue" (zero jobs)
# error that neither actionlint nor `task lint` can see -- they parse the file fine,
# so the failure only surfaces on a hosted run. GitHub does not document the exact
# ceiling; empirically a ~109 KB generated workflow ran fine, a 186 KB
# anaconda-publish.yaml was rejected at startup, and a 135 KB split of it still
# failed -- so the real limit sits just above 109 KB and at or below 135 KB. This cap
# is set below the known-bad 135 KB class so a bloat regression -- most likely a
# shared script re-inlined into every job, or a workflow accumulating too many
# scenarios in one file -- fails generation locally instead of silently at startup.
# The scenario suite is kept small by splitting it into one file per catalog workflow
# (each currently well under 25 KB); anaconda-publish itself is held under 100 KB
# (see tests/test_anaconda_publish.py). If a legitimate workflow ever approaches this
# cap, shrink or split it rather than raising the cap.
MAX_GENERATED_WORKFLOW_BYTES = 115_000
...
def render_published_workflow(item: Workflow) -> str:
    ...
    assert_within_size_cap(item.id, rendered)
    return rendered


def assert_within_size_cap(label: str, rendered: str) -> None:
    """Fail generation when a generated workflow exceeds the byte cap GitHub trips on.

    Covers both published catalog workflows (``label`` is the workflow id) and the
    generated scenario workflows (``label`` is the file name): a workflow can grow
    past the (undocumented) size GitHub accepts without any lint catching it --
    actionlint parses the file successfully; the run just fails at startup with a
    "workflow file issue" and zero jobs. Checking the rendered byte count here turns
    that invisible, hosted-only failure into a loud local generation error.
    """
    size = len(rendered.encode("utf-8"))
    if size > MAX_GENERATED_WORKFLOW_BYTES:
        raise DevflowsError(
            f"{label}: generated workflow is {size} bytes, over the "
            f"{MAX_GENERATED_WORKFLOW_BYTES}-byte cap. GitHub rejects oversized workflow "
            "files at startup with an opaque 'workflow file issue' (zero jobs) error that "
            "actionlint and `task lint` do not catch. Reduce the inlined script footprint "
            "-- e.g. split a shared module so each job inlines only the slice it needs."
        )

...
# Hard byte cap on a generated workflow file. GitHub rejects an oversized workflow
# at startup with an opaque "workflow file issue" (zero jobs) error that neither
# actionlint nor `task lint` can see -- they parse the file fine, so the failure only
# surfaces on a hosted run. GitHub does not document the exact ceiling; empirically
# pypi-publish.yaml (~69 KB) runs and a 186 KB anaconda-publish.yaml was rejected at
# startup (both an ASCII block-literal and an escaped-scalar rendering of that 186 KB
# file failed), so the real limit sits somewhere between 69 KB and 186 KB. This cap is
# set below the known-bad 186 KB and above the largest legitimate generated workflow
# (anaconda-publish, ~135 KB after its shared module was split so no single job inlines
# the whole thing) so that a bloat regression -- most likely a shared script re-inlined
# into every job -- fails `devflows sync` locally instead of silently at startup. If a
# legitimate workflow ever approaches this cap, shrink it (split its inlined scripts)
# rather than raising the cap; the value should trend down toward pypi's proven-safe
# ~69 KB, not up.
MAX_GENERATED_WORKFLOW_BYTES = 150_000
...
def render_published_workflow(item: Workflow) -> str:
    ...
    _assert_within_size_cap(item, rendered)
    return rendered


def _assert_within_size_cap(item: Workflow, rendered: str) -> None:
    """Fail sync when a generated workflow exceeds the byte cap GitHub trips on.

    A workflow can grow past the (undocumented) size GitHub accepts without any lint
    catching it -- actionlint parses the file successfully; the run just fails at
    startup with a "workflow file issue" and zero jobs. Checking the rendered byte
    count here turns that invisible, hosted-only failure into a loud local sync error.
    """
    size = len(rendered.encode("utf-8"))
    if size > MAX_GENERATED_WORKFLOW_BYTES:
        raise DevflowsError(
            f"{item.id}: generated workflow is {size} bytes, over the "

...
<owner>/<spec>`` for each owner-qualified target. The targets come from the verify
job's promoted-specs output (already validated and owner-prefixed there), so this
job never re-parses caller-controlled specs; it only executes the plan. The single
token-bearing step of the promote job.
...
def main() -> int:
    ...
    from_label = specs.validate_label(os.environ["UPLOAD_LABEL"], field="upload-label")
    to_label = specs.validate_label(os.environ["PROMOTE_LABEL"], field="promote-label")

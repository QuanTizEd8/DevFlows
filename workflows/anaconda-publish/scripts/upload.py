...
every file against the caller-supplied manifest one more time (a TOCTOU guard: the
credential-free verify job ran on a different runner, and python-build uploads its
artifacts with overwrite:true, so the artifact is not immutable within a run),
then runs ``anaconda upload`` for each verified file via an argv built by specs.py
(env + shlex, never a shell string). anaconda-client reads the token from the
inherited ANACONDA_API_TOKEN env, so it never lands in a process argument.
...
def main() -> int:
    ...
    owner = specs.validate_owner(os.environ["PUBLISH_OWNER"])
    server_url = os.environ.get("PUBLISH_SERVER_URL", "")
    label = specs.validate_label(os.environ["UPLOAD_LABEL"], field="upload-label")
    ...
    try:
        verified = specs.verify_files_against_manifest(dist_path, manifest)
        specs.resolve_version(verified, expected=os.environ.get("PUBLISH_EXPECTED_VERSION", ""))
